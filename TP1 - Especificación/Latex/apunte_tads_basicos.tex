...
\tadOperacion{HayEstudiante?}{posición/p, rastri/rs}{bool}
	{FiltrarValidas(Ag(p, $\emptyset$), rs) = Ag(p, $\emptyset$)}
\tadOperacion{HayHippie?}{posición/p, rastri rs}{bool}
{FiltrarValidas(Ag(p, $\emptyset$), rs) = Ag(p, $\emptyset$)}
\tadOperacion{HayHippie?}{posición/p, rastri rs}{bool}
{FiltrarValidas(Ag(p, $\emptyset$), rs) = Ag(p, $\emptyset$)}
\tadOperacion{HayAS?}{posición/p, rastri rs}{bool}
{FiltrarValidas(Ag(p, $\emptyset$), rs) = Ag(p, $\emptyset$)}
\tadOperacion{HayAlgo?}{posición/p, rastri rs}{bool}
{FiltrarValidas(Ag(p, $\emptyset$), rs) = Ag(p, $\emptyset$)}
